    ...
    <section id="encomendas" className="py-12 max-w-7xl mx-auto px-4 sm:px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        {COMMISSION_PACKAGES.map(pkg => (
          ...
          >
            {pkg.popular && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full bg-[#B64FFB] text-white text-[10px] font-bold font-mono uppercase tracking-wider shadow-sm">
                Mais Solicitado
              </div>
            )}
            ...
            <button
              ...
              style={{
                background: selectedPackage === pkg.id ? 'var(--text-accent)' : 'rgba(182,79,251,0.1)',
      ...
      <div className="navy-card p-8 sm:p-10 rounded-3xl max-w-2xl mx-auto">
        ...
        {submitted ? (
          <div className="p-6 rounded-2xl border text-center space-y-2"
            style={{ background: 'rgba(182,79,251,0.1)', borderColor: 'rgba(182,79,251,0.3)' }}
          >
            <div className="w-10 h-10 rounded-full bg-[#B64FFB] text-white font-bold text-lg flex items-center justify-center mx-auto">
              ✓
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs font-sans">
            ...
            <button type="submit"
              className="w-full py-3 rounded-2xl bg-[#B64FFB] hover:brightness-110 text-white font-bold text-xs uppercase tracking-wider shadow-lg active:scale-[0.98] transition-all"
            >
              Enviar Solicitação
            </button>
          </form>

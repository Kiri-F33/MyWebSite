    ...
    <section id="processo" className="py-8 max-w-7xl mx-auto px-4 sm:px-6">
      <div className="navy-card p-8 sm:p-12 rounded-3xl relative overflow-hidden">
        ...
        <div
          ...
          className="relative w-full max-w-4xl mx-auto aspect-[16/9] rounded-2xl overflow-hidden cursor-ew-resize select-none border border-white/10 shadow-2xl bg-black"
        >
          {/* AFTER (Final Artwork - Underneath) */}
          <div className="absolute inset-0 w-full h-full">
            <Image
              src="/images/fantasy_portrait.png"
              alt="Arte Finalizada"
              fill
              className="object-cover"
              priority
            />
            <div className="absolute top-4 right-4 bg-slate-900/80 backdrop-blur-md px-3 py-1 rounded-full border border-white/20 text-xs font-mono text-[#ABD2FA]">
              {process.badgeAfter}
            </div>
          </div>

          {/* BEFORE (Draft Sketch - Clipped Top Layer) */}
          <div
          ...
          >
            <div className="relative w-full aspect-[16/9] min-w-[300px] sm:min-w-[600px] md:min-w-[800px] h-full">
              <Image
                src="/images/charcoal_sketch.png"
                alt="Esboço Inicial"
                fill
                className="object-cover"
                priority
          ...
          {/* Slider Handle Line */}
          <div
            className="absolute top-0 bottom-0 w-1 bg-[#7692FF] shadow-[0_0_15px_rgba(118,146,255,0.8)] z-30"
            style={{ left: `${sliderPosition}%` }}
          >
            <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-10 h-10 rounded-full bg-[#7692FF] text-white shadow-lg flex items-center justify-center font-bold text-xs">
              &harr;
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-6 text-xs font-mono text-center" style={{ color: 'var(--text-body)' }}>
          ...
          <span className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-[#7692FF]"></span>
